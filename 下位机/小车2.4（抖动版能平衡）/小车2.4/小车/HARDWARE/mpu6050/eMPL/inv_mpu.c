...
#endif
volatile float yaw_temp=0;
volatile float last_yaw_temp=0;
...
//返回值:0,正常
//    其他,失败
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw,float *x_gyro,float *y_gyro,float *z_gyro)
{
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	unsigned long sensor_timestamp;
	short gyro[3], accel[3], sensors;  //x y z   /16.38
	unsigned char more;
	long quat[4]; 
	...
	**/  //180->-1880
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 180/PI;	// roll
		yaw_temp=atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
    ...
    *yaw  = (yaw_temp + yaw_count*360); 
		last_yaw_temp = yaw_temp;
		*x_gyro=gyro[0]/16.38f;    //转换成度每秒
		*y_gyro=gyro[1]/16.38f;
		*z_gyro=gyro[2]/16.38f;
	}else return 2;
	return 0;
}
